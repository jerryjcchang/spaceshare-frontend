  ...
  filterBySearchTerm = () => {
    ...
    return allSpaces.filter(
      space =>
        ...
        space.state.toLowerCase().includes(searchTerm.toLowerCase())
    );
  };

  filteredSpaces = () => {
    let allSpaces = this.filterBySearchTerm();
    return allSpaces.filter(this.filterSpace);
  };
      ...
      <InfiniteScroll
        ...
        loader={
          <Image
            centered
            size="medium"
            src="https://media.giphy.com/media/MtWKB3pR7vQeQ/giphy.gif"
          />
        }
        endMessage={
